...
- 现象：用户问“游园会的载体是啥”，文档里有“小程序、移动端”等描述，但 Self-RAG 最高分低于阈值，二次补救后仍拒答。
- 根因：用户说的是抽象概念“载体”，文档写的是具体承载形态“小程序、移动端、H5、网页、App”等；旧链路只有字面关键词和 hash embedding，不能稳定知道这些词属于同一类概念。
- 通用能力缺口：缺少“概念别名/语义字段”层，导致抽象问法和具体文档表达连不上。
