...
| 12 | Self-RAG | 回答前自检资料是否足够，剔除无用资料，不足时拒答。 | 第一阶段先强化 prompt 和证据过滤意识。 |
...
| Self-RAG | 部分完成 | Prompt 要求先判断资料是否支持问题，资料不足要拒答。 | `backend/app/rag.py` | 增加证据阈值；回答前先过滤低分 chunk；生成后做引用一致性检查。 |
...
2. 如果资料不足，明确回答“文档中没有找到依据”。

为什么现在只做轻量版：

- 真正的 Self-RAG 通常需要额外的判断步骤或模型调用。
- 当前项目优先保持教学可读性。
